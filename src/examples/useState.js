		...
		<div className='wrap'>
			<div className='top'>
				...
				<ul>
					...
					<li><p className='description'>инициализируя хук мы определяем сразу массив, в котором первый параметр это шаблон, а второй параметр функция которая будет его менять: <br /><b>const [<em>состояние, функция меняющая состояние</em>] = useState(<em>дефолтное значение</em>)</b></p></li>
					<li><p className='description'>мы не можем вставлять <b>useState</b> в какие-либо условия <b>if</b>, но можем делать проверки внутри хуков</p></li>
					...
					<li><p className='description'>когда мы используем useState в виде <b>объекта</b> в <span className='red'>функциональных</span> компонентах, то что мы передаем полностью заменяет что было, в этом отличие от <span className='red'>классовых</span> компонентов, поэтому надо указывать сохранение текущего объекта и только потом в хуке писать что изменить</p></li>
				</ul>
			</div>
